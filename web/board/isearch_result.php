...
?>
  ...
  <h4 style="margin-top:30px;"><a href="./inquiry_board.php">홈으로</a></h4>
...
if($catgo = "title" or "name" or "content")
{
    ...
    $param = "%{$_GET['search']}%";
    $sql2= "SELECT * from iboard where $catgo like ? order by idx desc";
    ...
    while($iboard= $member->fetch_assoc()){
      ...
      <tbody>
        <tr>
          <td width="70"><?php echo $iboard['idx']; ?></td>
          <td width="500">
            <?php 
          $lockimg = "<img src='/web/board/img/lock1.png' alt='lock' title='lock' width='20' height='20' /> ";
              if($iboard['lock_post']=="1")
              { ?><a href='/web/board/ck_iread.php?idx=<?php echo $iboard["idx"];?>'><?php echo $title, $lockimg;
              }else{?>

        <!--- 추가부분 18.08.01 END -->
        <a href='/web/board/iread.php?idx=<?php echo $iboard["idx"]; ?>'><?php echo $title; }?><span class="re_ct">[<?php echo $rep_count;?>]</span></a></td>
          <td width="120"><?php echo $iboard['name']?></td>
          <td width="100"><?php echo $iboard['date']?></td>
        </tr>
      </tbody>
    ...
    <!-- 18.10.11 검색 추가 -->
    <div id="search_box2">
      <form action="/web/board/isearch_result.php" method="get">
      <select name="catgo">
